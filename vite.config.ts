    ...
    plugins: [
      ...
      VitePWA({
        ...
        workbox: {
          ...
          runtimeCaching: [
            {
              urlPattern: ({ url }) => /(^|\.)(googleapis\.com|gstatic\.com|google\.com)$/.test(url.hostname),
              handler: 'NetworkOnly',
            },
            {
            ...
            {
              urlPattern: ({ request, url }) => url.origin === self.location.origin && ['script', 'style', 'font', 'image'].includes(request.destination),
              handler: 'CacheFirst',
              options: {
