...
(async () => {
  try {
    ...
    // Listen for load
    const loaded = new Promise(r => ws.once('message', m => { const o = JSON.parse(m.toString()); if (o.method === 'Page.loadEventFired') r(); }));
    await send('Page.navigate', { url: 'http://127.0.0.1:3000/' });
    await Promise.race([loaded, new Promise(r => setTimeout(r, 12000))]);
    await new Promise(r => setTimeout(r, 4000));
    const sheets = await send('Runtime.evaluate', { expression: 'document.querySelectorAll("#app-bottom-sheet").length', returnByValue: true });
    ...
    for (const e of events.slice(0, 20)) console.log(' ', e.level, '::', e.text.slice(0, 250));
