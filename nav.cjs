...
(async () => {
  ...
  // First close the edge://newtab page so we can navigate cleanly.
  const targets = await send('Target.getTargets');
  console.log('targets:', JSON.stringify(targets.targetInfos.filter(t => t.type === 'page').map(t => t.url)));
  for (const t of (targets.targetInfos || [])) {
    if (t.type === 'page' && t.url.startsWith('edge://')) {
      try { await send('Target.closeTarget', { targetId: t.targetId }); } catch (e) {}
    }
  }
  await new Promise(r => setTimeout(r, 500));
  const t2 = await send('Target.createTarget', { url: 'http://127.0.0.1:3000/' });
  console.log('new target:', t2.targetId);
